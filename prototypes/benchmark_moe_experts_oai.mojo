...
def section_combined[
    ...
    print_row(String(t"seq={seq_len}"), ks, ws, 0)
    if ws.p50 > 0:
        var gmac = Int64(seq_len) * TOP_K * 3 * Int64(HIDDEN) * INTERMEDIATE
        var gmac100 = (gmac * 100) // ws.p50
        var touched = min(Int64(seq_len) * TOP_K, Int64(NUM_EXPERTS))
        var wbytes = touched * Int64(WEIGHT_BYTES_PER_EXPERT)
        var gbs100 = (wbytes * 100) // ws.p50
        var ns_per_tok = ws.p50 // Int64(seq_len)
        print(
            t"      {gmac100 // 100}.{gmac100 % 100} GMAC/s | weights touched="
            t"{touched} -> {gbs100 // 100}.{gbs100 % 100} GB/s DRAM weight stream "
            t"(aggregate over active nodes) | {ns_per_tok} ns/token")
...
def run_all[P: BurstThreadPool, //](
    ...
    print(
        "\n=== M3 routed-expert path (phase1 SwiGLU-OAI + phase2 down) ===\n"
        "    low seq: memory-bound (weights streamed once); "
        "high seq: compute-bound (weights amortized over tokens)")
...
def main():
    ...
    var tp = len(topo)
    print("MiniMax-M3 MoE routed-expert (SwiGLU-OAI) benchmark")
